...
std::vector<uint8_t> MJPEGServer::demosaic_malvar(const uint16_t* bayer, int width, int height, int rawStride, int shift) {
...
std::vector<unsigned char> MJPEGServer::encode_jpeg(const libcamera::FrameBuffer* buffer) {
...
void MJPEGServer::handle_client(tcp::socket socket) {
    try {
        ...
        while (socket.is_open() && running_.load()) {
            ...
            last_frame = now;
            bool frame_ready = requestCV_.wait_for(lock, std::chrono::seconds(1), [&] {
                return std::any_of(requests_.begin(), requests_.end(), [](const auto& req) {
                    return req->status() == libcamera::Request::RequestComplete;
                });
            });
            if (!frame_ready)
                continue;
            // Check running flag after wait
            if (!running_.load())
                break;
